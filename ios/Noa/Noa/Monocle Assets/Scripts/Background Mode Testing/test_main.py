#
# Tests background mode by continuously sending queries every N minutes. The mic will just capture
# background noise.
#
# To deploy, use this in place of main.py and also include the other files. In Controller.swift,
# take care to modify transmitNextScript() so that it opens "main.py", as this name is required on
# Monocle in order to execute automatically.
#
...
# Touchpad disabled -- test will be automatic
...
while True:
    if state.current_state == state.Init:
        ...
        if state.on_entry():
            gfx.append_response(
                """Welcome to the Noa background mode tester.\nStart the Noa iOS or Android app."""
            )
        ...
        if state.on_entry():
            ...
            gfx.set_prompt("Waiting to send")
        if num_sent <= 0:
            # First one, send immediately
            state.after(1000, state.StartRecording)
        else:
            # Every 5 minutes otherwise
            state.after(5000 * 60, state.StartRecording)
